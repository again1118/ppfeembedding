...
namespace osuCrypto
{
    ...
    struct SimplestOTClientSession
    {
        ...
        void recvVector(uint8_t choice_bit, uint64_t* result_buffer, size_t length, int plainSocket, size_t &bytes_sent, size_t &bytes_recv)
        {
            // 1. 通过 OT 协议，根据选择位 (choice_bit) 只拿到对应的一把钥匙！
            BitVector choices(1);
            choices[0] = choice_bit;
            AlignedVector<block> receivedMsg(1);
            
            coproto::sync_wait(otReceiver.receiveChosen(
                choices, span<block>(receivedMsg.data(), 1), prng, sock));
            cp::sync_wait(sock.flush());
            
            // 拿到属于我的那把钥匙
            block mySeed = receivedMsg[0]; 
            ...
            // 2. 从普通 Socket 接收服务端发来的两个乱码箱子
            std::vector<uint64_t> cipher0(length), cipher1(length);
            NetworkHelper::recvUint64Array(plainSocket, cipher0.data(), length);
            NetworkHelper::recvUint64Array(plainSocket, cipher1.data(), length);

            // 3. 拿出我的钥匙，去解密我想要的那个箱子
            PRNG myPrng(mySeed);
            const std::vector<uint64_t>& chosenCipher = (choice_bit == 1) ? cipher1 : cipher0;
            
            for(size_t i = 0; i < length; i++) {
                result_buffer[i] = chosenCipher[i] ^ myPrng.get<uint64_t>(); // 异或解密，完美还原数据
            }
        }
    };
#else
...
int main(int argc, char *argv[])
{
    ...
    uint64_t Log2DBSize, EntrySize, plainModulus,embeddingdim;

    if (!NetworkHelper::recvData(serverSocket, &Log2DBSize, sizeof(Log2DBSize)) ||
        !NetworkHelper::recvData(serverSocket, &embeddingdim, sizeof(embeddingdim)) ||
    ...
    cout << "[Client] embeddingdim: " << embeddingdim << " bytes" << endl;
    ...
    OneSVClient client(Log2DBSize, embeddingdim, plainModulus);
    ...
    uint32_t TotalDBSize = 1 << Log2DBSize;
    cout << "[Client] Receiving encrypted database (" << TotalDBSize << " ciphertexts)..." << endl;
    ...
    if (!NetworkHelper::recvCiphertextVector(serverSocket, encryptedDB, TotalDBSize, context))
    {
    ...
    uint64_t *tempDB;
    initDatabase(&tempDB, Log2DBSize, embeddingdim, plainModulus); // Use same initialization function
    
    OneSVServer tempServer(tempDB, Log2DBSize, embeddingdim, context, keygen, plainModulus);
    ...
    uint64_t *result = new uint64_t[embeddingdim];
    ...
    for (uint32_t i = 0; i < num_queries; i++)
    {
        ...
        for (uint32_t idx = 0; idx < embeddingdim; idx++) {
            ran[idx] = getSecureRandom64() % plain_modulus_value;
        }
        troy::Plaintext ra = encoder.encode_polynomial_new(ran);
        ...
        if (!NetworkHelper::sendBatchQueries(serverSocket, singleQuery, PartNum, embeddingdim, context))
        {
        ...
        // 准备一个数组来装结果
        uint64_t* QueryResult = new uint64_t[embeddingdim];

        // 调用 OT 拿回真正的结果
        otSession.recvVector(resultBit, QueryResult, embeddingdim, serverSocket, ot_bytes_sent, ot_bytes_recv);
        ...
        // ==================== 【新增 1：接收 S_share 盲盒与后门】 ====================
        troy::Ciphertext ci_s_share;
        if (!NetworkHelper::recvCiphertext(serverSocket, ci_s_share, context)) {
            cerr << "[Client] Failed to receive encrypted S_share" << endl;
            break;
        }
        
        std::vector<uint64_t> test_server_share(embeddingdim);
        if (!NetworkHelper::recvUint64Array(serverSocket, test_server_share.data(), embeddingdim)) {
            cerr << "[Client] Failed to receive test S_share" << endl;
            break;
        }
        // ==========================================================================
        ...
        // ==================== 【修改 2：MPC 极简解盲与实时校验】 ====================
        std::vector<uint64_t> client_share(embeddingdim);
        std::vector<uint64_t> final_target(embeddingdim);
        uint64_t p = plain_modulus_value;
        bool current_query_correct = true;

        for (uint32_t l = 0; l < embeddingdim; l++) {
            // 1. 剥离 ran 掩码，得到干净的 C_share (注意：要在明文模数 p 下做减法)
            if(QueryResult[l] >= ran[l]) {
                client_share[l] = QueryResult[l] - ran[l]; 
            } else {
                client_share[l] = p + QueryResult[l] - ran[l];
            }
            
            // 2. 拼接 C_share 和 S_share 用于本地验证
            final_target[l] = (client_share[l] + test_server_share[l]) % p;

            // 3. 实时验证查到的 2048 维向量是否与数据库绝对一致
            uint64_t expected_val = tempDB[query * embeddingdim + l];
            if (final_target[l] != expected_val) {
            current_query_correct = false;
                cerr << "[Error] Query " << query << " Dim "<<l<<": Expected " << expected_val 
                        << ", Got " << final_target[l] 
                        << " (C:" << client_share[l] << " + S:" << test_server_share[l] << ")" << endl;
            break;
            }
        }

        if (current_query_correct) {
            cout << "[Success] Query " << query << " All correct!" << endl;
        }
        ...
        Plaintext pl_c_share = encoder.encode_polynomial_new(client_share); 

        client.ciParity[hintIndex] = evaluator.add_new(client.ciParity[parity_i], ci_s_share); 

        evaluator.add_plain_inplace(client.ciParity[hintIndex], pl_c_share);     
    ...
    if (access(options.OutputFile.c_str(), F_OK) == -1)
    {
        ...
        output_csv << "Variant, Log2 DBSize, EmbeddingDim, NumQueries, Offline Time (s), Online Time (ms), Amortized Compute Time Per Query (ms), Total Online Comm (MB)" << endl;
    }
    else
    {
    ...
    double online_total_comm_mb = (double)online_total_comm / (1024 * 1024);
    output_csv << "One server, " << Log2DBSize << ", " << embeddingdim << ", "
               << num_queries << ", " << (double)total_offline_time.count() / 1000.0 << ", "
               << online_time << ", " << amortized_compute_time_per_query<< online_total_comm_mb << endl;
