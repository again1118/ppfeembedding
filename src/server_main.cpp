...
ServerOptions parse_options(int argc, char *argv[])
{
    ...
    try
    {
        if (argc == 4)
        {
            ...
            options.embeddingdim = stoi(argv[2]);
...
namespace osuCrypto
{
    ...
    struct SimplestOTServerSession
    {
        ...
        void sendVector(const uint64_t* msg0, const uint64_t* msg1, size_t length, int plainSocket)
        {
            // 1. 生成两把 128 位的随机钥匙 (Seed)
            block seed0 = prng.get<block>();
            block seed1 = prng.get<block>();

            // 2. 通过纯正的 OT 协议，把这两把钥匙发出去
            AlignedVector<std::array<block, 2>> msgPairs(1);
            msgPairs[0][0] = seed0;
            msgPairs[0][1] = seed1;
            coproto::sync_wait(otSender.sendChosen(
                span<std::array<block, 2>>(msgPairs.data(), 1), prng, sock));
            cp::sync_wait(sock.flush());

            // 3. 用这两把钥匙作为流密码，把 2048 维数组加密成乱码
            PRNG prng0(seed0);
            PRNG prng1(seed1);
            std::vector<uint64_t> cipher0(length), cipher1(length);
            for(size_t i = 0; i < length; i++) {
                cipher0[i] = msg0[i] ^ prng0.get<uint64_t>(); // 异或加密
                cipher1[i] = msg1[i] ^ prng1.get<uint64_t>();
            }

            // 4. 把两个加密后的箱子，通过普通的 Socket 网络全扔给客户端
            NetworkHelper::sendUint64Array(plainSocket, cipher0.data(), length);
            NetworkHelper::sendUint64Array(plainSocket, cipher1.data(), length);
        }
    };
#else
    struct SimplestOTServerSession
...
int main(int argc, char *argv[])
{
    ...
    cout << "Embedding Size: " << options.embeddingdim << " 维" << endl;
    ...
    while (bgvRingSize < options.embeddingdim && bgvRingSize <= 32768)
    {
        bgvRingSize *= 2;
    }

    if (bgvRingSize < options.embeddingdim || bgvRingSize > 32768)
    {
        cerr << "[Error] Embedding dimension B (" << options.embeddingdim << ") exceeds maximum supported bgvRingSize (32768)" << endl;
        cerr << "[Error] Please decrease your EntrySize parameter" << endl;
        return 1;
    }
    // ==============================================================================

    cout << "[Server] PartSize=" << PartSize << ", EmbeddingDim=" << options.embeddingdim << ", bgvRingSize=" << bgvRingSize << endl;
    ...
    initDatabase(&DB, options.Log2DBSize, options.embeddingdim, plainModulus);
    cout << "[Server] Database initialization complete" << endl;

    // Create server object
    OneSVServer server(DB, options.Log2DBSize, options.embeddingdim, context, keygen, plainModulus);
    ...
    NetworkHelper::sendData(clientSocket, &options.embeddingdim, sizeof(options.embeddingdim));
    ...
    uint64_t *Response_b0 = new uint64_t[options.embeddingdim];
    uint64_t *Response_b1 = new uint64_t[options.embeddingdim];
    ...
    while (true)
    {
        ...
        { // Batch query mode
            ...
            {
                batchResponses[i].Response_b0.resize(options.embeddingdim);
                batchResponses[i].Response_b1.resize(options.embeddingdim);
                ...
                server.onlineQuery(bvec_array, batchQueries[i].Svec.data(),
                                   ...
                                   batchQueries[i].ciphertext, encoder,encryptor,ci_s_share);
                ...
                otSession.sendVector(batchResponses[i].Response_b0.data(), 
                     batchResponses[i].Response_b1.data(), 
                     options.embeddingdim, clientSocket);
                // ==================== 【新增：网络发送 S_share】 ====================
                // 1. 发送服务端份额的密文盲盒 (用于后续状态机融合)
                NetworkHelper::sendCiphertext(clientSocket, ci_s_share, context);
                // 2. 发送明文 server_share 后门 (仅作我们在本地终端校验 All correct 使用)
                NetworkHelper::sendUint64Array(clientSocket, server.server_share.data(),options.embeddingdim);
                // ====================================================================
        ...
        { // Single query mode (queryType == 0x01)
            ...
            // Process query
            troy::Ciphertext ci_s_share;
            server.onlineQuery(bvec, Svec, Response_b0, Response_b1, queryCiphertext, encoder,encryptor,ci_s_share);

            // Send response
            if (!NetworkHelper::sendUint64Array(clientSocket, Response_b0, options.embeddingdim))
            {
                cerr << "[Server] Failed to send Response_b0" << endl;
                break;
            }

            if (!NetworkHelper::sendUint64Array(clientSocket, Response_b1, options.embeddingdim))
            {
